...
export default function TeacherForm() {
  ...
  const fetchTeacher = async () => {
    ...
    try {
      ...
      setTeacher(response.data.filter((teacher) => teacher.role == "teacher"));
      setFilteredTeacher(response.data.filter((teacher) => teacher.role == "teacher"));
    } catch (err) {
  ...
  useEffect(() => {
    if (searchTerm.trim() === "") {
      ...
      const filtered = teacher.filter((student) =>
  ...
  const handleSaveEdit = async (studentId) => {
    try {
      ...
      // Update local state
      const updatedTeacher = teacher.map((student) =>
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">View and search registered teacher</p>
      </div>
      ...
      <div className="mb-4 text-sm text-gray-600">
        {searchTerm ? (
          <span>
            Found {filteredTeacher.length} student(s) matching "{searchTerm}"
          </span>
        ) : (
          <span>Total Teachers: {teacher.length}</span>
        )}
      </div>
